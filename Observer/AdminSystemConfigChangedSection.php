...
class AdminSystemConfigChangedSection implements ObserverInterface
{
    ...
    const CSS_RELATION_ARRAY = [
        'merge_css_files',
        'minify_files',
    ];

    const HTML_RELATION_ARRAY = [
        'minify_html',
    ];

    const DATABASE_RELATION_ARRAY = [
    ...
    public function execute(Observer $observer)
    {
        ...
        foreach ($groups['css'] as $key => $value) {
            if (in_array($key, self::CSS_RELATION_ARRAY)) {
                $developerCssFields['fields'][$key] = ['value' => $value];
            }
        }

        foreach ($groups['html'] as $key => $value) {
            if (in_array($key, self::HTML_RELATION_ARRAY)) {
                $developerHtmlFields['fields'][$key] = ['value' => $value];
            }
        }

        foreach ($groups['database'] as $key => $value) {
            if (in_array($key, self::DATABASE_RELATION_ARRAY)) {
                $developerDbFields['fields'][$key] = ['value' => $value];
            }
        }

        $developerGroup['css'] = $developerCssFields;
        $developerGroup['template'] = $developerHtmlFields;
        $this->proceedTransaction($developerGroup, 'dev');

        $developerGroup['frontend'] = $developerDbFields;
        $this->proceedTransaction($developerGroup, 'catalog');
    }
